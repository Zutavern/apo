...
// Konstanten
const PORTRAIT_BUCKET = 'bg_offers-pt'
const LANDSCAPE_BUCKET = 'bg_offers-ls'
const MAX_FILE_SIZE = 14 * 1024 * 1024 // 14MB
const ITEMS_PER_PAGE = 3

// Hilfskomponenten bleiben gleich wie in emergency/backgrounds/page.tsx
...
// Hauptkomponente
export default function OfferBackgrounds() {
  const supabase = createClientComponentClient()
  const router = useRouter()
  
  // State
  const [portraitImages, setPortraitImages] = useState<OfferBackground[]>([])
  const [landscapeImages, setLandscapeImages] = useState<OfferBackground[]>([])
  ...
  // Delete Handler
  const handleDelete = async (image: OfferBackground) => {
    try {
  ...
  // Select Handler
  const handleSelect = async (image: OfferBackground) => {
    try {
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <div className="flex justify-between items-center mb-8">
        <h1 className="text-2xl font-bold text-gray-100">Angebote Hintergründe</h1>
        <Button
          variant="outline"
          onClick={() => router.back()}
        ...
        {portraitImages.length === 0 ? (
          <EmptyState 
            ...
            onUpload={() => {
              ...
              input.onchange = (e) => {
                const files = (e.target as HTMLInputElement).files
                if (files?.length) handleFilesSelected(files, 'portrait')
              }
              input.click()
            }} 
          />
        ...
        {landscapeImages.length === 0 ? (
          <EmptyState 
            ...
            onUpload={() => {
              ...
              input.onchange = (e) => {
                const files = (e.target as HTMLInputElement).files
                if (files?.length) handleFilesSelected(files, 'landscape')
              }
              input.click()
            }}
          />
