...
// Konstanten
const PORTRAIT_BUCKET = 'bg-notdienst-pt'
const LANDSCAPE_BUCKET = 'bg-notdienst-ls'
...
// Hauptkomponente
export default function EmergencyBackgrounds() {
  const supabase = createClientComponentClient()
  const router = useRouter()
  
  // State
  const [portraitImages, setPortraitImages] = useState<EmergencyBackground[]>([])
  const [landscapeImages, setLandscapeImages] = useState<EmergencyBackground[]>([])
  ...
  // Delete Handler
  const handleDelete = async (image: EmergencyBackground) => {
    try {
  ...
  // Select Handler
  const handleSelect = async (image: EmergencyBackground) => {
    try {
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <div className="flex justify-between items-center mb-8">
        <h1 className="text-2xl font-bold text-gray-100">Notdienst Hintergründe</h1>
        <Button
          variant="outline"
          onClick={() => router.back()}
